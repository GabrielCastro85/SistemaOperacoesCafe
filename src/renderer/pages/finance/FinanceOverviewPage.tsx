...
export function FinanceOverviewPage({ data }: FinancePageProps): JSX.Element {
  ...
  return (
    <section className="content-section">
      ...
      <PageSection title="Vencimentos proximos" description="Contas abertas ordenadas para acompanhamento operacional."><PayablesTable payables={upcoming} categories={finance.categories} costCenters={finance.costCenters} legalEntities={data.legalEntities} locations={data.locations} onOpen={(id) => { window.location.hash = `#/finance/payables/${id}`; }} onEdit={(id) => { window.location.hash = `#/finance/payables/${id}/edit`; }} onConfirm={(id) => void window.operationsCafe.confirmAccountPayable(id).then(reload)} onPay={() => { window.location.hash = "#/finance/payments"; }} onCancel={(id) => void cancel(id)} /></PageSection>
    </section>
  );
}
