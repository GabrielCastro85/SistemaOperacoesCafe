...
export function FinancialProjection({ summary }: { summary: FinancialSummary | null }): JSX.Element {
  ...
  return (
    <PageSection title="Entradas e saidas projetadas" description="Fluxo gerencial projetado, sem representar saldo bancario real.">
      <ProgressBar value={Math.round((receivable / total) * 100)} label={`Recebimentos previstos ${formatCurrencyFromCents(receivable)}`} />
      <ProgressBar value={Math.round((payable / total) * 100)} label={`Pagamentos previstos ${formatCurrencyFromCents(payable)}`} />
    </PageSection>
  );
}
