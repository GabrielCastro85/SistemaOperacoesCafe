...
export function ChargeDetailsPage({ detail }: { detail: ClientChargeDetail | null }): JSX.Element {
  return <PageHeader eyebrow="Cobranca" title={detail ? chargePeriodLabel(detail.charge) : "Detalhe da cobranca"} description={detail ? `${detail.charge.status} - ${formatCurrencyFromCents(detail.charge.finalAmountCents)}` : "Selecione uma cobranca para abrir os detalhes."} />;
}
