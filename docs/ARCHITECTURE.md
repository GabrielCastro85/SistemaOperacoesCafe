...
Na decima etapa, o renderer foi reorganizado. `src/renderer/App.tsx` passou a ser apenas o ponto de montagem React. Layout, navegacao, temas e componentes reutilizaveis foram movidos para `app`, `layouts` e `design-system`. Esta etapa nao altera SQLite, migrations ou IPCs.

Na continuacao da etapa 10, os modulos operacionais e de confirmacoes foram migrados para `src/renderer/pages/operations`, `src/renderer/pages/imports` e `src/renderer/pages/confirmations`.

Na etapa 10.2, `src/renderer/app/App.tsx` passou a executar o bootstrap, manter o contexto ativo, envolver as paginas com `AppLayout` e resolver diretamente as rotas hash.

Na continuacao da etapa 10, os modulos comerciais e de recebimentos tambem foram migrados. As paginas diretas ficam em `src/renderer/pages/partners`, `products`, `serviceRates`, `charges` e `clientLedger`, com subpastas `components`, `forms` e `hooks` para separar lista, detalhes, formularios e leitura de dados. O roteamento continua em `src/renderer/app/App.tsx` e nao foram adicionadas migrations, SQL ou IPCs nesta migracao visual.
...
Na conclusao da etapa 10, o workspace legado foi removido. O frontend final fica organizado por dominio em `pages/finance`, `pages/settings`, `pages/operations`, `pages/imports`, `pages/confirmations`, `pages/partners`, `pages/products`, `pages/serviceRates`, `pages/charges` e `pages/clientLedger`. Financeiro e configuracoes usam hooks por modulo, componentes especificos de negocio dentro da propria pasta e componentes genericos do design system como `TreeView`, `CalendarGrid`, `DefinitionList`, `Timeline`, `PageSection`, `MonthInput` e `PercentageInput`.
