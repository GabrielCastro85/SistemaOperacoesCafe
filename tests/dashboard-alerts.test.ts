...
describe("dashboard alerts", () => {
  ...
  it("stops flagging a deal confirmation once it is cancelled, even though it was waiting for signature", async () => {
    const { repo, db, partnerId, productId } = await setup();
    const seller = await repo.createBusinessPartner({ organizationId: villaId, displayName: "Villa Vendedora", notes: null, roles: ["SELLER"], isActive: true });

    const draft = repo.createDealConfirmationDraft({ organizationId: villaId, ownLegalEntityId, confirmationDate: "2026-07-01", paymentTermsSnapshot: "A vista", deliveryLocationSnapshot: "Armazem", qualityTermsSnapshot: "Padrao", generalTermsSnapshot: "Revisado" });
    ...
    repo.addDealConfirmationParty({ dealConfirmationId: draft.confirmation.id, partyRole: "BUYER", businessPartnerId: partnerId, partnerLegalEntityId: null, ownLegalEntityId: null, manualName: null, representativeName: null, sortOrder: 2 });
    repo.addDealConfirmationItem({ dealConfirmationId: draft.confirmation.id, sortOrder: 0, productId, productNameSnapshot: "Cafe", productDescriptionSnapshot: "Cafe arabica", cropSnapshot: "2026", qualitySnapshot: "Bebida dura", packagingSnapshot: "Sacas de juta", originSnapshot: "Sul de Minas", destinationSnapshot: "Armazem", quantitySacksDecimal: "10", sackWeightKgDecimal: "60", unitPriceDecimal: "1000", totalAmountCents: null, totalOverrideReason: null, deliveryStartDate: "2026-07-05", deliveryEndDate: "2026-07-10", deliveryLocationSnapshot: "Armazem", notes: null });
    ...
    db.prepare("UPDATE deal_confirmations SET sent_for_signature_at = ? WHERE id = ?").run("2020-01-01T00:00:00.000Z", issued.confirmation.id);
    expect(repo.getDashboardAlerts(villaId, ownLegalEntityId).waitingSignatureConfirmations).toHaveLength(1);

    const cancelled = repo.cancelDealConfirmation(issued.confirmation.id, "Cliente desistiu");
    expect(cancelled.confirmation.signatureStatus).toBe("NOT_APPLICABLE");
    expect(repo.getDashboardAlerts(villaId, ownLegalEntityId).waitingSignatureConfirmations).toHaveLength(0);
    db.close();
  });
  ...
  it("flags a loan with a collection date already due, and stops once it is marked as collected -- substitui os lembretes soltos em .txt", async () => {
    const { repo, db, partnerId } = await setup();
    const loan = repo.createLedgerEntry({ organizationId: villaId, ownLegalEntityId, clientPartnerId: partnerId, clientChargeId: null, entryType: "LOAN", effect: "INCREASE_RECEIVABLE", amountCents: 2500000, entryDate: "2026-07-01", description: "Emprestimo", referenceNumber: null, notes: null, attachmentPath: null, availableAmountCents: null, status: "CONFIRMED", collectionDueDate: "2020-01-06" });

    const future = repo.createLedgerEntry({ organizationId: villaId, ownLegalEntityId, clientPartnerId: partnerId, clientChargeId: null, entryType: "LOAN", effect: "INCREASE_RECEIVABLE", amountCents: 100000, entryDate: "2026-07-01", description: "Emprestimo futuro", referenceNumber: null, notes: null, attachmentPath: null, availableAmountCents: null, status: "CONFIRMED", collectionDueDate: "2099-01-01" });

    const alerts = repo.getDashboardAlerts(villaId, ownLegalEntityId);
    expect(alerts.loansDueForCollection).toHaveLength(1);
    expect(alerts.loansDueForCollection[0]).toMatchObject({ ledgerEntryId: loan.id, partnerId, partnerName: "Cliente Alertas", amountCents: 2500000, collectionDueDate: "2020-01-06" });
    expect(alerts.loansDueForCollection[0].daysOverdue).toBeGreaterThan(365);

    const collected = repo.markLoanCollected(loan.id);
    expect(collected.collectedAt).not.toBeNull();
    expect(repo.getDashboardAlerts(villaId, ownLegalEntityId).loansDueForCollection).toHaveLength(0);
