...
describe("transfer reconciliations", () => {
  it("reuses a note in partial reconciliations and never creates billing movements", async () => {
    ...
    try {
      ...
      repo.saveTransferReconciliation({ organizationId, clientPartnerId: partner.id, referenceDate: "2026-09-18", notes: "Parcela final", status: "COMPLETED", invoices: [{ fiscalDocumentId: doc.document.id, sourceAmountCents: 200_000_00 }], payments: [{ beneficiaryName: "Maria", amountCents: 210_000_00 }] });
