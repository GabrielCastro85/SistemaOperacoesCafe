-- Segunda coluna ambigua na mesma funcao (0023 corrigiu confirmation_number,
-- mas nao pegou esta): "returns table (confirmation_number text,
-- sequence_number bigint)" tambem torna "sequence_number" uma variavel de
-- saida. A consulta que calcula v_highest_reservation usava
-- "max(sequence_number)" sem qualificar a tabela -- confirmado com um teste
-- transacional (rollback, nada persistido) direto contra producao: "column
-- reference sequence_number is ambiguous" em toda tentativa de reservar um
-- numero novo (nao idempotente).
begin;
