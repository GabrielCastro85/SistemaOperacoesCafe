-- Corrige bug real que estava bloqueando toda previa/emissao de confirmacao
-- em producao: a funcao ensure_deal_confirmation_number (0021) declara
-- "returns table (confirmation_number text, sequence_number bigint)", o que
-- torna "confirmation_number" um nome de variavel de saida dentro do corpo da
-- funcao. A verificacao final (`perform 1 from deal_confirmations where ...
-- and confirmation_number = v_number`) usava o nome sem qualificar a tabela,
-- e o Postgres nao consegue decidir entre a coluna da tabela e a variavel de
-- saida -- erro "column reference confirmation_number is ambiguous" em toda
-- chamada. Mesma funcao, so' com a tabela explicitamente referenciada (dc.)
-- na verificacao final.
begin;
...
begin
  ...
  -- Verificação final dentro da mesma transação. "dc." explicito e' o que
  -- corrige o bug -- sem isso "confirmation_number" bate com a variavel de
  -- saida da funcao (returns table), nao com a coluna da tabela.
  perform 1
  from public.deal_confirmations dc
