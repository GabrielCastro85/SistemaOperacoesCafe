...
async function buildCompactConfirmationPdf(input: Parameters<typeof generateDealConfirmationPdf>[0]): Promise<Uint8Array> {
  ...
  const columns = [
    { label: "Produto", x: margin + 8, width: 260, align: "left" as const },
    { label: "Sacas", x: margin + 278, width: 62, align: "right" as const },
    { label: "R$/saca", x: margin + 350, width: 86, align: "right" as const },
    { label: "Total", x: margin + 446, width: contentWidth - 454, align: "right" as const }
  ];
  page.drawRectangle({ x: margin, y: currentY - 15, width: contentWidth, height: 15, color: headerColor });
  columns.forEach((column) => {
    if (column.align === "right") {
      drawRightText(page, column.label, column.x, currentY - 10.5, column.width, bold, 6.8, headerText);
    } else {
      page.drawText(column.label, { x: column.x, y: currentY - 10.5, size: 6.8, font: bold, color: headerText });
    }
  });
  currentY -= 15;
...
export async function buildConfirmationPdf(input: Parameters<typeof generateDealConfirmationPdf>[0]): Promise<Uint8Array> {
  ...
  ensureSpace(44);
  const columns = [
    { label: "Produto", x: margin + 8, width: 245, align: "left" as const },
    { label: "Sacas", x: margin + 263, width: 62, align: "right" as const },
    { label: "R$/saca", x: margin + 335, width: 88, align: "right" as const },
    { label: "Total", x: margin + 433, width: contentWidth - 441, align: "right" as const }
  ];
  page.drawRectangle({ x: margin, y: y - 16, width: contentWidth, height: 16, color: dark });
  columns.forEach((column) => {
    if (column.align === "right") {
      drawRightText(page, column.label, column.x, y - 11, column.width, bold, 7, rgb(1, 0.96, 0.88));
    } else {
      page.drawText(column.label, { x: column.x, y: y - 11, size: 7, font: bold, color: rgb(1, 0.96, 0.88) });
    }
  });
  y -= 18;
  detail.items.forEach((item) => {
